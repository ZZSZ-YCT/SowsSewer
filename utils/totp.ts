...
function n(a: any, b: number[]): void {
    ...
    for (c = 0; c <= 79; c++) {
        if (c >= 16) {
            l[c] = ((l[c - 3] ^ l[c - 8] ^ l[c - 14] ^ l[c - 16]) << 1) |
...
/**
 * TOTP 函数：输入 Base32 秘钥 K，返回当前 6 位动态验证码
 */
export function generateTOTP(K: string): string {
    const C = Math.floor(new Date().getTime() / 30000);
    const key = sjcl.codec.base32.toBits(K);
    // count 为 64 位，JS 仅处理 32 位整数
    const count = [Math.floor(C / 0x100000000), C & 0xffffffff];
    const otplength = 6;
    ...
    const otpNum = dbc2 % Math.pow(10, otplength);
    let result = otpNum.toString();
